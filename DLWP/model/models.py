...
class SmartDataGenerator(Sequence):
    ...
    memory, but does so efficiently by leveraging its knowledge of the predictor-target sequence and time_step
    dimension. DO NOT USE if the predictors and targets are not a continuous time sequence where dt between samples
